# This script performs the "focused instrument selection" method described in the paper:
...
# Inputs:
# bx : p-vector of univariable genetic variant associations with the exposure
# sx : p-vector of standard errors corresponding to the genetic variant-exposure associations bx
# by : p-vector of univariable genetic variant associations with the outcome
# sy : p-vector standard errors corresponding to the genetic variant-outcome associations by
# S0 : the indices of the p-vector bx that form the "core" set of instruments thought to be valid
# k0: the number of clusters to form additional instruments sets (select 1 if only selecting between the core and the full set of instruments)
# alpha: the nominal coverage probability of confidence intervals is 1-alpha
# gamma: the worst-case coverage probability of focused confidence intervals is 1-alpha-gamma

# Outputs:
# core : the core estimator
# core.se : the standard error of the core estimator
# core.ci : (1-alpha)*100% core confidence interval
# focused : the focused estimator
# onestep.ci : (1-alpha)*100% "1-step" confidence interval
# focused.ci : (1-alpha)*100% "focused" confidence interval
# twostep.ci : (1-alpha)*100% "2-step" confidence interval
# additional.ivs : number of additional instruments selected
# core.CP : concentration parameter for the core set of instruments
# focused.CP : concentration parameter for the focused-selected set of instruments
# winner: selected set of instruments
# naive.se : the (naive) standard error of the selected estimator
...
focusedMR <- function(bx,by,sx,sy,V,k0,alpha,gamma){
 ...
 # shortest one-step interval according to a minimum coverage constraint
  ...
  res.list <- list("core"=tet_v, "core.se"=sqrt(var_v), "core.ci"=c(tet_v-qnorm(1 - alpha/2)*sqrt(var_v),tet_v+qnorm(1 - alpha/2)*sqrt(var_v)), "focused"=tet_ps, "onestep.ci"=onestep.ci, "focused.ci"=onestep.min, "twostep.ci"=twostep.ci, "additional.ivs"=((ifelse(sel==1,1,0)*0)+(ifelse(sel>1,1,0)*length(S[[which.min(amse)]]))), "core.CP"=CP_V <- sum(pmax((Bx[V]^2)-(Sx[V]^2),0)/(Sx[V]^2))/length(V), "focused.CP"= sum(pmax((Bx[S[[which.min(amse)]]]^2)-(Sx[S[[which.min(amse)]]]^2),0)/(Sx[S[[which.min(amse)]]]^2))/length(S[[which.min(amse)]]), "winner"=sel0, "naive.se"=sqrt(naive.var))
 return(res.list)
}
